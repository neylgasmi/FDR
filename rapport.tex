% rapport_efdr.tex — Sequential jump detection at high frequency
% Compile with pdflatex or lualatex
% Authors: Neyl Gasmi & Jules Brion — M1 MAEF 2025-2026
% ============================================================
...
% ============================================================
\title{%
  \textbf{Sequential jump detection at high frequency\\
  via e-values and online false-discovery-rate control}\\[0.5em]
  {\large TER 2025-2026}
}
\author{%
...
(Andersen-Dobrev-Schaumburg, 2012) and feed them into five online FDR
procedures from the Ramdas school (e-BH, e-LOND, e-LORD, e-SAFFRON, stopped
e-BH). Monte-Carlo experiments (\texttt{grid\_medium}, $M=100$ replications,
$n=500$ steps, $3$ frequencies, $3$ jump regimes) show that the BH-based
benchmarks (Yen, 2013; Bajgrowicz-Scaillet, 2016) violate the criterion
...
frequencies, with a power loss of $10$--$21$ percentage points.
\end{abstract}
...
\begin{enumerate}
  \item \textbf{E-value construction for HF jump detection}: integrated
    ...
    (\texttt{evalues/construct.py}), valid under arbitrary dependence.
...
(\texttt{test\_fdr\_algorithms.py}); the GROW
construction is required to dominate this calibrator in e-power.
\end{remark}
...
\begin{align*}
  \FDP &= \frac{F_D}{R_D \vee 1}
    && \text{(false discovery proportion)},\\
  \FDR &= \E^P[\FDP]
    && \text{(false discovery rate)},\\
  \FWER &= \Prob(F_D \geq 1)
    && \text{(family-wise error rate)},\\
  \mathrm{mFDR}(\eta) &= \frac{\E^P[F_D]}{\E^P[R_D] + \eta}
    && \text{(marginal FDR)},
\end{align*}
with the convention $0/0 = 0$ encoded by $R_D \vee 1$ in the denominator.
...
\paragraph{Parameters} (\texttt{simulate/heston.py}, lines 89--106):
\begin{equation}
  \kappa = 2.0,\quad \theta = 0.04,\quad \xi = 0.5,\quad \rho = -0.7,\quad
  V_0 = 0.04,\quad \mu = 0.
\end{equation}
...
Implementation: \texttt{simulate/heston.py}, \texttt{\_qe\_kernel}.
...
(\texttt{detector.py}):
\begin{equation}
  \lambda_\text{rare} = 1\,500/\mathrm{year} \approx 0.64\text{ jumps/path},
  \quad
  \lambda_\text{moderate} = 5\,000/\mathrm{year} \approx 2.12\text{ jumps/path}
\end{equation}
...
An error is raised if $m \ge 1$ (\texttt{hawkes.py}, lines 59--65).
...
\begin{equation}\label{eq:medrv}
  \mathrm{MedRV}_n = c_\mathrm{med} \cdot \frac{n}{n-2}
    \sum_{i=2}^{n-1} \mathrm{med}(|r_{i-1}|, |r_i|, |r_{i+1}|)^2,
  \quad c_\mathrm{med} = \frac{\pi}{6 - 4\sqrt{3} + \pi} \approx 1.4191.
\end{equation}
\end{definition}

\begin{definition}[MinRV, ADS 2012 eq.~13]
\begin{equation}\label{eq:minrv}
  \mathrm{MinRV}_n = c_\mathrm{min} \cdot \frac{n}{n-1}
    \sum_{i=2}^{n} \min(|r_{i-1}|, |r_i|)^2,
  \quad c_\mathrm{min} = \frac{\pi}{\pi - 2} \approx 2.7141.
\end{equation}
\end{definition}

Implemented in \texttt{estimators/threshold.py} (43 l.).

\subsection{Local spot volatility and causality condition}
\label{sec:spot}

\texttt{estimators/spot.py} (76 l.) computes $\hat{\sigma}^2(i)$ over a
...
The window is $K = 50$ in the pipeline (\texttt{EstimatorConfig.window = 50},
\texttt{detector.py} line 71). The first $K$ hypotheses are excluded from
...
Implemented in \texttt{estimators/bipower.py} via
\texttt{lee\_mykland\_stat}.
...
\begin{equation}\label{eq:evalue_integral}
  E_i = \int_{-\infty}^{+\infty}
    \exp\!\left(\frac{J r_i}{v} - \frac{J^2}{2v}\right)
    \cdot \frac{1}{\sqrt{2\pi\tau^2}}\exp\!\left(-\frac{J^2}{2\tau^2}\right) dJ.
\end{equation}
...
\begin{equation}\label{eq:evalue}
  \boxed{E_i = \sqrt{\frac{v}{v + \tau^2}} \cdot
    \exp\!\left(\frac{r_i^2\,\tau^2}{2\,v\,(v + \tau^2)}\right)},
  \quad v = \dt\,\hat{\sigma}_i^2,\quad \tau = 5\,\hat{\sigma}_i\sqrt{\dt}.
\end{equation}
...
Four unit tests (\texttt{test\_evalues\_validity.py}, 223 l.) verify:
\begin{enumerate}
...
\[
  k^* := \max\!\left\{k \in \{1, \ldots, K\} : \frac{k\,e_{[k]}}{K} \geq \frac{1}{\alpha}\right\}
  \qquad (k^* := 0 \text{ if the set is empty}).
\]
\end{definition}

If $k^* = 0$, no hypothesis is rejected. The
complexity is $O(K \log K)$ for sorting, implemented in \texttt{fdr/ebh.py}
(44 l.).
...
\begin{proof}
Let $D$ be a self-consistent e-testing procedure, and let
$E = (E_1, \ldots, E_K)$ be the input e-variables. By self-consistency, for
every $k \in D$, $E_k\,R_D \geq K/\alpha$, hence on the event $\{R_D \geq 1\}$:
\[
  \frac{\ind_{\{k \in D\}}}{R_D} \leq \frac{\alpha\,E_k}{K}.
\]
Therefore the FDP of $D$ satisfies
\[
  \frac{FD}{R_D \vee 1}
    = \frac{|D(E) \cap N|}{R_D \vee 1}
    \leq \sum_{k \in N} \frac{\ind_{\{k \in D(E)\}}\,\alpha\,E_k}{K}
    \leq \sum_{k \in N} \frac{\alpha\,E_k}{K}.
\]
The first inequality applies self-consistency conditionally on $k \in D$.
Taking expectations:
\[
  \E\!\left[\frac{FD}{R_D \vee 1}\right]
    \leq \sum_{k \in N} \frac{\alpha\,\E[E_k]}{K}
    \leq \frac{\alpha\,K_0}{K},
\]
since $\E[E_k] \leq 1$ for $k \in N$.

The e-BH procedure is self-consistent by construction: for $k \in D$,
$e_k \geq e_{[R_D]} \geq K/(\alpha R_D)$ by definition of $k^* = R_D$.
\end{proof}
...
$\sum_{t=1}^{10^6} \gamma_t \approx 1.000000$ (\texttt{fdr/elond.py}).
...
\begin{proof}
Bounding the indicator $\delta_j = \ind_{\{E_j \geq 1/\alpha_j\}}$ by
$\alpha_j E_j$ (Markov), and using $R_t \vee 1 \geq R_{j-1} + 1$ for
$j \leq t$ contributing to $R_t$:
\begin{align*}
\FDR(t) &= \E\!\left[\frac{\sum_{j \in H_0(t)} \delta_j}{R_t \vee 1}\right]
  \leq \E\!\left[\sum_{j \in H_0(t)} \frac{\delta_j}{R_{j-1}+1}\right]\\
  &= \E\!\left[\sum_{j \in H_0(t)} \frac{\ind_{\{E_j \geq 1/\alpha_j\}}}{R_{j-1}+1}\right]
  \leq \E\!\left[\sum_{j \in H_0(t)} \frac{E_j\,\alpha_j}{R_{j-1}+1}\right]\\
  &= \E\!\left[\sum_{j \in H_0(t)} \frac{\E[E_j \mid \mathcal{F}_{j-1}]\,\alpha_j}{R_{j-1}+1}\right]
  \leq \E\!\left[\sum_{j \in H_0(t)} \frac{\alpha_j}{R_{j-1}+1}\right]\\
  &= \E[FDP^*(t)] \leq \alpha,
\end{align*}
where we used $\alpha_j$ and $R_{j-1}$ are $\mathcal{F}_{j-1}$-measurable, so
that $\E[E_j \mid \mathcal{F}_{j-1}] \leq 1$ pulls out.
\end{proof}
...
\E[\widehat{FDP}_{\text{e-SAFFRON}}(t)]
  &= \sum_{j=1}^{t} \E\!\left[\frac{\alpha_j}{R_{j-1}+1} \cdot \frac{\ind_{\{e_j < 1/\lambda_j\}}}{1 - \lambda_j}\right]\\
  &\geq \sum_{j \in H_0(t)} \E\!\left[\frac{\alpha_j}{R_{j-1}+1}
      \cdot \frac{\E[\ind_{\{e_j < 1/\lambda_j\}} \mid \mathcal{F}_{j-1}]}{1 - \lambda_j}\right].
\end{align*}
Since $E_j$ is an e-variable and $\lambda_j$ is $\mathcal{F}_{j-1}$-measurable,
Markov gives $\Prob(E_j \geq 1/\lambda_j \mid \mathcal{F}_{j-1}) \leq \lambda_j$
under $H_{0,j}$, hence
$\Prob(E_j < 1/\lambda_j \mid \mathcal{F}_{j-1}) \geq 1 - \lambda_j$.
Therefore
\[
  \E[\widehat{FDP}_{\text{e-SAFFRON}}(t)]
    \geq \sum_{j \in H_0(t)} \E\!\left[\frac{\alpha_j}{R_{j-1}+1}\right]
    = \E[FDP^*(t)].
\]
\end{proof}
...
\paragraph{BH-LM.}
\texttt{bh\_lee\_mykland} (\texttt{baselines.py} lines 39--58) computes
$L_i = r_i / (\hat{\sigma}_i\sqrt{\dt})$, the two-sided p-value
$P_i = 2\Prob(|Z| > |L_i|)$, then applies BH. Decisions are made per
hypothesis.

\paragraph{BH-BNS.}
In \texttt{detector.py} (lines 170--177), BH-BNS calls
\texttt{bh\_bns\_global} (global BNS ratio test, \texttt{baselines.py}
lines 104--120) and duplicates its result across all hypotheses: if the
global test rejects, all returns are flagged as jumps; otherwise none. This
matches the BS 2016 usage (BNS as a day-level filter).
...
\[
  ...
  \quad
  \text{Delay} = \frac{1}{|\mathcal{R} \cap \mathcal{H}_1|}
    \sum_{i \in \mathcal{R} \cap \mathcal{H}_1}(t_i - t_{i,\text{true}}).
\]
...
\paragraph{Interpretation.}
BH's FDR violation at $5\,\mathrm{s}$ results from BV's bias under dense
jumps: BV under-estimates the spot volatility when products
$|r_{i-1}||r_i|$ are contaminated, inflating the normalised statistic and
producing false positives. This mechanism is structural --- it concerns the
BV normalisation, not additive noise.
...
\[
  E_i = \int_{-\infty}^{+\infty}
    \exp\!\left(\frac{Jr_i}{v} - \frac{J^2}{2v}\right)
    \cdot \frac{1}{\sqrt{2\pi\tau^2}}\exp\!\left(-\frac{J^2}{2\tau^2}\right) dJ.
\]
...
The Gaussian integral gives $\int \frac{1}{\sqrt{2\pi\tau^2}} e^{-(J-\mu^*)^2/(2s^2)} dJ = s/\tau$, hence:
\[
  E_i = \frac{s}{\tau} \exp\!\left(\frac{\mu^{*2}}{2s^2}\right)
  = \sqrt{\frac{v}{v+\tau^2}} \cdot \exp\!\left(\frac{r_i^2\tau^2}{2v(v+\tau^2)}\right).
\]
...
\[
  ...
  \qquad C_\mathrm{JM} = 0.15708906,
  \qquad \sum_{t=1}^{10^6} \gamma_t \approx 1.0000.
\]
